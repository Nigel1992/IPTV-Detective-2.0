...
function parse_m3u($text) {
    $lines = preg_split('/\r?\n/', $text);
    $lines = array_filter(array_map('trim', $lines), function($l){ return $l !== ''; });
    ...
    for ($i = 0; $i < count($lines); $i++) {
        ...
        if (stripos($line, '#EXTINF') === 0) {
            // get name
            $name = '';
            if (preg_match('/#EXTINF:[^,]*,(.*)/i', $line, $m)) {
                $name = trim($m[1]);
            }
            // attributes
            $attrs = [];
            if (preg_match_all('/(\w[\w-]*)=\"([^\"]*)\"/', $line, $am, PREG_SET_ORDER)) {
            ...
            $url = '';
            $j = $i + 1;
            while ($j < count($lines) && strpos($lines[$j], '#') === 0) $j++;
            if ($j < count($lines)) $url = $lines[$j];
            $channels[] = ['name' => $name, 'group' => $group, 'url' => $url];
            $i = $j;
        }
    }
    return ['groups' => array_keys($groups), 'channels' => $channels];
}
?>