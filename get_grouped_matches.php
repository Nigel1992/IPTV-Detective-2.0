<?php
// get_grouped_matches.php - Return provider groups where similarity >= 95%
    ...
    for ($i = 0; $i < $n; $i++) {
        for ($j = $i+1; $j < $n; $j++) {
            ...
            foreach ($metricCols as $col => $baseW) {
                $valA = isset($providers[$i][$col]) ? intval($providers[$i][$col]) : 0;
                $valB = isset($providers[$j][$col]) ? intval($providers[$j][$col]) : 0;
